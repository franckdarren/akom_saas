...
export function MenuItem({ product }: MenuItemProps) {
    ...
    return (
        <motion.div
            ...
            className={`flex items-center gap-2 bg-card p-3 shadow-sm ${isOutOfStock ? 'opacity-60' : ''
                }`}
        >
            {/* Image */}
            <div className="relative w-16 h-16 sm:w-20 sm:h-20 shrink-0 bg-muted rounded-lg overflow-hidden">
                {product.imageUrl ? (
                    <Image
